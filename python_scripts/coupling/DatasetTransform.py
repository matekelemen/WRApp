...
#class AcceleratorDatasetTransform(DatasetTransform):
#    """ @brief Apply a convergence accelerator on a variable.
#        @classname AcceleratorDatasetTransform
...
#        self.__CheckInputParameters()
#
#        # Try constructing accelerators from FSIApplication
#        try:
...
#            # Fall back to CoSim accelerators
...
#
#        # Construct a separate convergence accelerator for each target variable
#        accelerator_parameters = self._parameters["accelerator"]
#        self._accelerators: "dict[str,KratosMultiphysics.ConvergenceAccelerator]" = {
#            target_params["variable_name"].GetString() : CreateConvergenceAccelerator(accelerator_parameters)
#            for target_params in self._parameters["target"]["variables"].values()
#        }
#
#
...
#                   source_expression,
#                   target_params: KratosMultiphysics.Parameters) -> None:
#        accelerator = self._accelerators[target_params["variable_name"].GetString()]
#
#
#    def __CheckInputParameters(self) -> None:
#        for target_params in self._parameters["target"]["variables"].values():
#            if target_params["entity_type"] != KratosMultiphysics.ContainerType.NodalHistorical:
#                raise ValueError(f"Convergence acceleration is only valid for historical variables\n{self._parameters}")
